...
def check_chemspeed_setup():
    """Various protocols to check before running the chemspeed script (ensure simple mistakes are not left behind). Returns true if everything is setup appropiatly"""
    protocol_check = [
        input('Has the CH2Cl2 vial been added and topped up? (1=Yes, 0=No): '),
        input('Is there enough CHCN3 left in the resevoir? (1=Yes, 0=No): '),
        input('Is the special waste empty enough for one run? (1=Yes, 0=No): '),
        input('Have all 48 reaction vials been added in the right places? (1=Yes, 0=No): '),   
        input('Have all NMR tubes been added, and are the two green stickers aligned? (1=Yes, 0=No): '),  
        input('Have all the MS vials been added? (1=Yes, 0=No): '),
        input('Have all the measured masses been added to the stock_space csv? (1=Yes, 0=No): '),
        input('Have the stock vials been added in the right locations? (1=Yes, 0=No): '),  
        input('Is Jeff still fun to banter with? (1=Yes, 0=No): '),  
        ]

    check_total = 0
    for check in protocol_check:
        check = check
        try:
            if int(check) == 1:
                check_total += 1
            ...
            pass
    if check_total == len(protocol_check):
        return True
    
    else:
        print('')
        print('Check set up and rerun get_batch_spaces_csvs()')
        return False
...
def get_batch_spaces_csvs():
    ...
    flag = check_chemspeed_setup()                                             #checking that the chemspeed platform is setup as intended (its easy to forget things like setting up CHCl2 vial)
    ...
    if flag == True:                                                            
        ...
        #Generating and saving the NMR json and sending it via email
        batch_space.save_NMR_JSON()
        batch_space.send_NMR_json_to_email()
