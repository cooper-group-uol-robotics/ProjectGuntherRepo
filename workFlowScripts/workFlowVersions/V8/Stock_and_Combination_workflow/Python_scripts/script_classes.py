...
class USER_SELECTION():
    ...
    generated_json_path = str_cwd + '/Generated_NMR_JSONs/'
    generated_MS_csv_path = str_cwd + '/Generated_MS_CSV/'
    metal_concentration = 20 * 1.5                                                                 #The concentration of the metal in the stock vials, as if it had a coordination number of 6 (this attribute can be easily implited as a class attribute in the metal, but I thought doing this allows for more flexibility)
...
class Batch_space():
    ...
    def save_NMR_JSON(self):
        ...
        path = USER_SELECTION.generated_json_path + 'NMR_json.json'
        json_to_save = {}
    ...
    def save_MS_csv(self):
        ...
        for reaction_indx, reaction in enumerate(self.sample_space):                                                #Iterating through the reactions in the batch space and adding its sample parameters to the csv_dct 
            text_var = f'{self.batch_number}_{reaction.unique_identifier}'
        ...
        path = USER_SELECTION.generated_MS_csv_path + 'Gunther_MS_csv.csv'                                          #Reading the dictionary as a dataframe and saving it as a csv
    ...
    def send_NMR_json_and_MS_csv_to_email(self):
        ...
        json_path = USER_SELECTION.generated_json_path + 'NMR_json.json'
        csv_path = USER_SELECTION.generated_MS_csv_path + 'Gunther_MS_csv.csv'
        try:
            ol = win32com.client.Dispatch('Outlook.Application')                                                    #sending stock space to print via outlook
            olmailitem=0x0
            newmail = ol.CreateItem(olmailitem)
