...
class USER_SELECTION():
    ...
    generated_csv_path =  str_cwd + '/Generated_CSVs/'
    generated_pickle_path = str_cwd + '/Generated_pickles/'
    metal_concentration = 20                                                                 #The concentration of the metal in the stock vials, as if it had a coordination number of 6 (this attribute can be easily implited as a class attribute in the metal, but I thought doing this allows for more flexibility)
    dialdehyde_concentration = 0
    diamine_concentration = 30
    monoaldehdye_concentration = 60
    reaction_vial_transfer_volume = 200                                                      #The volume in microlitres to transfer from stock vials to sample vials (again, this could be added as an attribute to the chemical class, but it does give up flexibility)
    ...
    batch_size = 48
    standard_reaction_idx = 0                                                                #This is the index of where the standard reaction is found in the reaction space (Reaction_space.add_standard_reaction() method is called)
    


class Chemical():
    """Parent chemical class with data relaventy to all chemicals"""
    def __init__(self, name: str, CAS:str, molecular_weight:int, solubility:tuple, price:float, volume_in_reaction_vial:float, concentration_in_stock_vial:float) -> None:
        self.name = name                                                                     #name 
...
class Metal(Chemical):
    ...
    def __init__(self, name: str, CAS: str, molecular_weight: int, solubility: tuple, price: float, volume_in_reaction_vial: float, concentration_in_stock_vial: float, no_coordination_sites: tuple, selected_coordinaiton_num:int = None) -> None:
        super().__init__(name, CAS, molecular_weight, solubility, price, volume_in_reaction_vial, concentration_in_stock_vial)
...
class Dialdehyde(Chemical):
    ...
    def __init__(self, name: str, CAS: str, molecular_weight: int, solubility: tuple, price: float, volume_in_reaction_vial: float, concentration_in_stock_vial: float, no_coordination_sites:int, no_imine_reaction_sites: int) -> None:
        super().__init__(name, CAS, molecular_weight, solubility, price, volume_in_reaction_vial, concentration_in_stock_vial)
        ...
        for chemical in self.space:
            ...
            if chemical_subtype not in self.divided_space.keys() and chemical_subtype == 'Metal':            #as before, however with metals, each different coordination number is seen as a 'unique metal' (this will make combination calculations for reactions with same metal but different CN more easy)
                ...
                if type(chemical.no_coordination_sites) == int:                                              #checking that if the tuple is interpreted as an integer or tuple by python
                    ...
                    new_chemical = Metal(name=chemical.name, CAS=chemical.CAS, molecular_weight=chemical.molecular_weight, solubility=chemical.solubility, price=chemical.price, volume_in_reaction_vial=chemical.volume_in_reaction_vial, concentration_in_stock_vial= chemical.concentration_in_stock_vial, no_coordination_sites=chemical.no_coordination_sites, selected_coordinaiton_num=coordination_number)
...
class Reaction():
    ...
    def __init__(self, final_reaction_volume:float, reagents:list, whole_space:Whole_chemical_space) -> None:
...
class Reaction_space():
    ...
    def generate_reaction_space(self):
        ...
        for reagent_combo in self.reagent_space:
            reaction = Reaction(final_reaction_volume=1500, reagents=reagent_combo, whole_space=self.whole_space )
    ...
    def add_standard_reaction(self, reaction_idx):
        ...
        reaction = reaction_space[reaction_idx]
    ...
    def save_to_csv(self):
        """save the reaction space to a csv"""
        dct_to_convert = {}
        columns_names = ['Reaction'] + [reagent.name for reagent in self.whole_space.space] + ['CH3CN']                   #columns are: reaction name + reagents + solvents (order is of utmost importance)
        path = USER_SELECTION.generated_csv_path + 'reaction_space.csv'
        for reaction_idx, reaction in enumerate(self.reaction_space):
            dct_to_convert[f'{reaction_idx}'] = [str(reaction.reagents)]+reaction.volume_of_reagents_required
...
class Batch_space():
    ...
    def save_to_sample_csv(self):
        """saves the smaple space to a Gunther readable csv"""
        dct_to_convert = {} 
        columns_names = ['Reaction']+[reagent.name for reagent in self.whole_chemical_space.space] + ['CH3CN']                      #columns are: reaction name + reagents + solvents (order is of utmost importance)
    ...
    def get_stock_space(self):
        ...
        for chemical_index, chemical in enumerate(self.whole_chemical_space.space):                                                 #iterating through the chemicals
            ...
            chemical_Mw = chemical.molecular_weight
            chemical_concentration = chemical.concentration_in_stock_vial/1000                                                      #divide by 1000 to get moles
