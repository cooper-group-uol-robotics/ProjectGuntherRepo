...
class ReactionSpace():
    ...
    def addStandardReaction(self, reactionIdx):
        """Adds a standard reaction to the first sample of each batch."""
        
        batchSize = USERSELECTION.batchSize                                                                                            #The standard reaction is a user defined variable.
        reactionSpace = self.reactionSpace
        
        #Getting the attributes of the standard reaction.
        
        standardReaction = reactionSpace[reactionIdx]                                                                                  #Retrieving the standard reaction. Along with its attributes.
        ...
        multipleInt = 0                                                                                                                #Iterating via integer batch steps to find the index of where to append the repeated standard reaction.
        multiple = 0
        while multiple + batchSize < len(reactionSpace):
            multiple = multipleInt * (batchSize)
            ...
            reaction.unique_identifier = str(USERSELECTION.standardReactionIdx)+'batch'+str(multipleInt)
            
            reactionSpace.insert(multiple, reaction)                                                                                   #Adding the standard reaction to the appropiate location.
            multipleInt = multipleInt + 1
...
class BatchSpace():
    ...
    def __init__(self, batchSize:int) -> None:
        ...
        self.batchNumber = 0                                                                                                           #This is the current batch number.
        self.batchTaken = False                                                                                                        #This is a flag. It ensure the batch number will change only if both 'takeNewBatch()' and 'getBatchSpacesCsvs()' functions have been run in main.
        self.WholeChemicalSpace = None                                                                                                 #This is the whole chemical space.
        self.ReactionSpace = None                                                                                                      #This is the reaction space.
        self.stockSpace = None                                                                                                         #This is the stockspace. It handles the formation of stock solutions. 
        self.sampleSpace = None                                                                                                        #A list with all the reaction present (as reaction objects) in the batch (these are the combinations to take in this batch).
    ...
    def setBatchFlagToTrue(self):
        """If the batch has been taken it sets True."""
        
        self.batchTaken = True
    
    def setBatchFlagToFalse(self):
        """If the batch has been taken it sets False."""
        
        self.batchTaken = False

    def changeBatchNumber(self):
        """Changes the batch number depending on the batch flag."""
        
        if self.batchTaken == True:                                                                        
            self.batchNumber += 1                   
            self.batchTaken = False
    ...
    def getSampleSpace(self):
        ...
        currentBatchNumber = self.batchNumber                                                                                          
        reactionSpaceSize = self.ReactionSpace.getSpaceSize()                                                                          #gets the size of the reaction space.
        finalBatchNumber = math.ceil(reactionSpaceSize/USERSELECTION.batchSize)-1
        print(f'The current batch number gunther is running is {currentBatchNumber}. (Batches numbers start from 0). The last batch number is {finalBatchNumber}')
        
        if currentBatchNumber > finalBatchNumber:                                                                                      #Checks if all the reactions in the reaction space have been completed.
            self.sampleSpace = []
            print('ALL REACTIONS DONE, EXPERIMENT FINISHED')        
        
        else:
            if currentBatchNumber == finalBatchNumber:                                                                                 #Checks that the upcoming batch does not go over the reaction space size (this is important form a programmatic standpoint -> prevents an out of range error).
                self.sampleSpace = self.ReactionSpace.reactionSpace[currentBatchNumber*self.batchSize:]

            else:
                self.sampleSpace = self.ReactionSpace.reactionSpace[currentBatchNumber*self.batchSize:(currentBatchNumber+1)*self.batchSize]
    ...
    def saveSampleSpaceToCsv(self):
        ...
        pathCsv = USERSELECTION.generatedCsvPath + 'sampleSpace.csv'
        pathLogs = USERSELECTION.logsPath + str(self.batchNumber) + '_sampleSpace.csv'
        
        for reactionIdx, reaction in enumerate(self.sampleSpace):
            DictionaryToConvert[f'{reactionIdx}'] = [reaction.unique_identifier] + [str(reaction.reagents)] + reaction.volumeOfReagentsRequired
    ...
    def calcStockMasses(self):
        ...
        for chemical in self.WholeChemicalSpace.space:                                                                                  #Iterating through the chemicals in the whole chemical sapce and checks their presence in the batch space.
            chemicalVolume = 0
            for reaction in self.sampleSpace:                                                                                           #Getting the total volume of a chemical in the batch space.
    ...
    def saveNmrJson(self):
        ...
        for reaction_num, reaction in enumerate(self.sampleSpace):
                ...
                "nmr_experiments": [
                    {
                        ...
                        "numScans": USERSELECTION.numScans,
                        "pp_threshold": USERSELECTION.ppThershold,
                        "fieldPresat": USERSELECTION.fieldPresat
                    }
                ]
            } 
    ...
    def saveMsCsv(self):
        ...
        for reaction_indx, reaction in enumerate(self.sampleSpace):                                                                     #Iterating through the reactions in the batch space and adding its sample parameters to the csvDictionary.
